...
- **Errors:** raise a subclass of `TuriyaError` (`src/turiya/errors.py`) for any expected failure. Let `cli.py`'s single `except TuriyaError` handler print the message and set the exit code — don't `print`/`sys.exit` directly from `operations/*`.
- Build all JSON output with `json.dumps` — never string-concatenated JSON.
- Log via `StructuredLogger.emit_event` / `.log_human` / `.run_start` / `.run_end` from `src/turiya/logging.py` — don't write to a log file directly, and don't invent a parallel logging mechanism.
...
```
Files under `logging.dir`: `ops.jsonl` (combined), `<op>.jsonl` (per-op), `<op>.log` (human-readable). `logging.json_per_file = false` in the config suppresses only `file` events.
